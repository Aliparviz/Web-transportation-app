...
using namespace std;
std::vector<Driver*> Application:: get_drivers(){return drivers;}
std::vector<Passenger*> Application::get_passengers(){return passengers;}
std::vector<Location*> Application::get_locations(){return locations;}
std::vector<Travel*> Application::get_travels(){return travels;}
void Application:: add_id() { id_counter++; }
Application::Application(char* path)
{
    Application::read_file(path);
}
...
void Application::sign_up(std::string username, std::string role)
{
...
void Application::post_trip(string user_name, string org, string dest, string check_in_hurry)
{
    bool is_hurry = false;
...
std::vector<Travel*> Application::sort_travels_by_cost()
{
...
std::vector<Travel*>  Application::sort_travels_by_id()
{
...
void Application::get_all_trips(std::string username, std::string sort)
{
    Driver *driver = find_driver_by_name(username);
...
bool Application::check_delete_trip(Passenger *passenger, Travel *travel)
{
    ...
    else if (travel->get_status() == traveling || travel->get_status() == finished || travel->get_status() == "canceled")
    {
        throw std::runtime_error(BAD_REQUEST);
    }
    return true;
}
void Application::delete_trip(int id, string username)
{
